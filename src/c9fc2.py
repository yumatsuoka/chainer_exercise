...
class C9fc2(FunctionSet):
    """CNNのモデルの１つ"""
    def __init__(self, output_dim):
        super(C9fc2, self).__init__(
                ...
                conv9 = F.Convolution2D(32, 32, 3, pad=1),
                #Linear(入力次元,出力次元)
                fc1 = F.Linear(8192, 4096),
                fc2 = F.Linear(4096, output_dim)
        )
    ...
    def forward(self, x_data, y_data, train=True, gpu=-1):
        ...
        h = F.max_pooling_2d(F.relu(self.conv6(h)), ksize=2, stride=2)
        h = F.relu(self.conv7(h))
        h = F.relu(self.conv8(h))
        h = F.max_pooling_2d(F.relu(self.conv9(h)), ksize=2, stride=2) 
